...
import { getClothProduct, getFeaturedProduct, getTechProduct } from '../Services'
import Footer from '../Components/GlobalComponents/Footer'
import { useNavigate } from 'react-router-dom'

function TechProduct() {
  const {data} = useQuery(['product'], getTechProduct);
  const newData = data?.products?.slice(0,20);
                    ...
                    <div key={product.id} className="col-lg-3 col-md-4 col-sm-6">
                      <div className='indivitual-product-wrapper' >
                        <div className='indivitual-product-image'>
                          <img  onClick={()=> navigate(`/product-details/${product.id}`)} src={product.images[0]} alt={product.title} className='img-fluid' />
                        </div>
                        <div className='indivitual-product-title text-center' >
                          <h6 onClick={()=> navigate(`/product-details/${product.id}`)} >{product.title}</h6>
